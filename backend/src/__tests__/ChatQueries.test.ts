...
describe("getChatsByUserId", () => {
    ...
    const createMockChats = (count: number): Chat[] => {
        return Array.from(
            { length: count },
            () => new Chat(userRequester, userHelper, ad)
        );
    };

    it("should return two chats for user requester", async () => {
        const mockChats = createMockChats(2);
        mockTypeOrm().onMock(Chat).toReturn(mockChats, "find");

        const chatsRetrieved = await chatQueries.getChatsByUserId(
            userRequester.id!
        );
    ...
    it("should return an empty array if the user has no chats", async () => {
        ...
        mockTypeOrm().onMock(Chat).toReturn([], "find");
